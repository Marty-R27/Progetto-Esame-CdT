...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="tei:person" mode="info-card">
    <div class="view_info" id="{(@xml:id, generate-id())[1]}">
      ...
      <h3><xsl:value-of select="if ($name != '') then $name else 'Senza nome'"/></h3>
  ...
  <xsl:template match="tei:place" mode="info-card">
    <div class="view_info" id="{(@xml:id, generate-id())[1]}">
      <xsl:variable name="placeText" select="normalize-space(string-join(tei:placeName/text(), ' '))"/>
      <xsl:variable name="place" select="if ($placeText != '') then $placeText else normalize-space(tei:placeName/tei:settlement[1])"/>
      
      <h3><xsl:value-of select="if ($place != '') then $place else 'Luogo non identificato'"/></h3>
  ...
  <xsl:template match="tei:biblStruct" mode="info-card">
    <div class="view_info" id="{(@xml:id, generate-id())[1]}">
      ...
      <h3><xsl:value-of select="if ($title != '') then $title else 'Titolo non disponibile'"/></h3>
  ...
  <xsl:template match="tei:term" mode="info-card">
    <div class="sub_info {@subtype}">
      <h4>Categoria: <xsl:value-of select="@subtype"/></h4>
      <div class="note-text">
        <p>
          <xsl:choose>
  ...
  <xsl:template match="tei:zone[@rend='line']" mode="overlay">
    <!-- Estraggo l'ID pulito per permettere al JS di collegare la riga SVG al testo.
         Dal tuo XML, l'ID è es: "Farfalla_1877_4_8_90_lg1_l1". 
         Rimuovendo il prefisso, l'ID dell'SVG diventa: "line-lg1_l1". -->
    <rect id="line-{substring-after(@xml:id, 'Farfalla_1877_4_8_90_')}" class="line-overlay"
          x="{@ulx}" y="{@uly}"
  ...
  <xsl:template match="tei:pb">
    <div class="page-break" id="{@xml:id}">
      <!-- Usa @xml:id perché @facs ora punta alla surface (senza numero pagina) -->
      <span>[Pagina <xsl:value-of select="substring-after(@xml:id, 'Farfalla_1877_4_8_')"/>]</span>
    </div>
  </xsl:template>
  ...
  <!-- BLOCCO 5: Testo, Poesia, Parole e Abbreviazioni -->
  
  <!-- NUOVO: Gestione delle Strofe -->
  <xsl:template match="tei:lg">
    <div class="stanza {@xml:id}">
      <xsl:apply-templates/>
    </div>
    <br/> <!-- Aggiunge uno spazio per staccare visivamente le strofe -->
  </xsl:template>
  
  <!-- NUOVO: Gestione dei Versi (Sostituisce il vecchio compito di lb) -->
  <xsl:template match="tei:l">
    <!-- Estraiamo l'ID togliendo il prefisso col cancelletto (#). Passiamo anche @rend (es. "indent") al CSS -->
    <div id="text-{substring-after(@facs, '#Farfalla_1877_4_8_90_')}" class="text-line {@rend}">
      <xsl:apply-templates/>
    </div>
  </xsl:template>
  
  <!-- Manteniamo la regola per i paragrafi (nel caso ci fossero introduzioni in prosa) -->
  ...
  <!-- Semplifichiamo lb (se dovesse capitare una riga spezzata non interattiva) -->
  ...
  <xsl:template match="tei:signed">
    <div class="firma {@rend}">
      ...
      <xsl:if test="@facs">
        <span id="text-{substring-after(@facs, 'Farfalla_1877_4_8_90_')}" class="text-line"></span>
      </xsl:if>
  ...
  <!-- 2. Entità Interattive (Collegate ai file JS tramite gli attributi ref e target) -->
  ...
  <!-- 4. Elementi Tipografici Generici e Varie (dal modello originale) -->
